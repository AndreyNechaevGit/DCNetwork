...
1. В Вашем плане я бы рекомендовал для Loopback интерфейсов коммутаторов Spine придерживаться подхода, который использовался для Spine-1, то есть 10.8.X.0/32, где X - номер спайна. На Spine-2 эта логика немного съехала.
2. Для Loopback коммутаторов Leaf я бы рекомендовал использовать адреса типа 10.8.0.X/32, где X - номер лифа, так точно не будет путаницы с адресами спайнов.
